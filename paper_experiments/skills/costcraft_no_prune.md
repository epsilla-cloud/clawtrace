...
3. **Plan the deliverable structure before writing.** Identify what columns /
   ...
   reference files. For spreadsheets, read the workbook first to confirm
   header rows, data ranges, and key-to-column mappings before writing
   formulas or code.
...
- [ ] **Delimiter precision:** when joining values (TEXTJOIN, Python join, etc.), default to bare comma `","` with no trailing space unless the task explicitly requests spaces. Mismatched separators cause silent validation failures.
- [ ] **N/A formatting:** when marking cells as not applicable, write `N/A` (with slash). Do not use `NA`, `na`, `n/a`, or other variants unless the task specifies otherwise.
