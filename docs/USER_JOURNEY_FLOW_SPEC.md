...
| F3 Daily Control Room | Returning user or scheduled run | Portfolio + selected cockpit | "Are we healthy today?" | User either stays in monitor mode or escalates to investigation | F4/F5 |
...
4. Ask user for high-impact confirmations only.
5. Publish initial control posture.
...
3. Open drawer only when investigation is needed.
...
3. Distinguish likely runtime failure vs state drift.
