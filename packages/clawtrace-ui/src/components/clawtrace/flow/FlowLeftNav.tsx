...
const FUNCTIONAL_NAV_ITEMS: FunctionalNavItem[] = [
  {
    ...
    icon: 'setup',
    flowIds: ['f0-connect', 'f1-audit', 'f2-handoff'],
  },
  {
    id: 'daily-operations',
    label: 'Daily Operations',
    icon: 'operations',
    flowIds: ['f3-control-room', 'f4-live-run'],
  },
  {
    id: 'diagnose-issues',
    label: 'Diagnose Issues',
    icon: 'diagnose',
    flowIds: ['f5-triage', 'f9-time-machine'],
  },
  {
    id: 'resolve-verify',
    label: 'Resolve & Verify',
    icon: 'resolve',
    flowIds: ['f6-intervention', 'f7-verification'],
  },
  {
    id: 'prevention-eval',
    label: 'Prevention & Eval',
    icon: 'prevent',
    flowIds: ['f8-regression'],
  },
  {
    id: 'automation',
    label: 'Automation',
    icon: 'automate',
    flowIds: ['f10-automation'],
  },
  {
    id: 'feedback-loop',
    label: 'Feedback Loop',
    icon: 'feedback',
    flowIds: ['f11-feedback'],
  },
];
...
function renderIcon(icon: NavIcon) {
  switch (icon) {
    case 'setup':
      return (
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M13 2v6m-2 0h4M6 10V8a2 2 0 0 1 2-2h1m9 4V8a2 2 0 0 0-2-2h-1M6 14v2a2 2 0 0 0 2 2h2m8-4v2a2 2 0 0 1-2 2h-2M8 12h8" />
        </svg>
      );
    case 'operations':
      return (
    ...
    case 'diagnose':
      return (
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M11 4a7 7 0 1 0 4.95 11.95L20 20m-6-7h.01" />
        </svg>
      );
    case 'resolve':
      return (
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="m4 20 6.5-6.5m0 0L13 11l3 3 4-4m-9.5 3.5L7 10m6-6 2 2m-8 8 2 2" />
        </svg>
      );
    case 'prevent':
      return (
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M12 3 5 6v5c0 5 3.5 8.5 7 10 3.5-1.5 7-5 7-10V6zM9 12l2 2 4-4" />
        </svg>
      );
    case 'automate':
      return (
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M5 6h14v10H8l-3 3zM9 10h6m-6 3h4" />
        </svg>
      );
    case 'feedback':
      return (
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M4 12a8 8 0 0 1 14-5m2-3v6h-6M20 12a8 8 0 0 1-14 5m-2 3v-6h6" />
        </svg>
      );
    default:
      return null;
...
export function FlowLeftNav({ flow, allFlows }: FlowLeftNavProps) {
  ...
  const items = useMemo(
    () =>
      FUNCTIONAL_NAV_ITEMS.map((item) => ({
        ...item,
        route: resolveRoute(item.flowIds, allFlows),
      })),
    [allFlows],
  );

  const activeId = useMemo(
    () => items.find((item) => item.flowIds.includes(flow.id))?.id ?? null,
    [flow.id, items],
  );
  ...
  return (
    <aside className={`${styles.nav} ${expanded ? styles.expanded : styles.collapsed}`} aria-label="ClawTrace functional navigation">
      <button
        type="button"
      ...
      <div className={styles.currentMeta}>
        ...
        <span className={styles.currentFlowText}>{flow.title}</span>
      </div>
    </aside>
  );
}
