# ClawTrace

ClawTrace is observability for OpenClaw agents: see failures, cost leaks, and the next action to improve reliability.

## Product Surfaces

- `packages/clawtrace-ui` — Next.js app (landing, overview, tracing detail, Tracy copilot)
- `openclaw-tracing` — upstream tracing reference implementation cloned for integration work
- [OpenClaw Tracing reference README](openclaw-tracing/README.md)

## Design + Planning Docs

- [Design System](DESIGN.md)
- [CEO Product Spec](docs/CEO_PRODUCT_SPEC.md)
- [User Journey Flow Spec](docs/USER_JOURNEY_FLOW_SPEC.md)
- [Trace Detail Concept Renaming Spec](docs/TRACE_DETAIL_CONCEPT_RENAMING_SPEC.md)
- [OpenClaw Cost Audit Control Journey](docs/OPENCLAW_COST_AUDIT_CONTROL_JOURNEY.md)
- [OpenClaw State Versioning Research](docs/OPENCLAW_STATE_VERSIONING_RESEARCH.md)
- [Agent Evaluation Strategy](docs/AGENT_EVALUATION_STRATEGY.md)
- [Observability Market Research](docs/OBSERVABILITY_MARKET_RESEARCH.md)
- [Workflow Reliability Wedge Design](docs/WORKFLOW_RELIABILITY_WEDGE_DESIGN.md)
- [Landing Page Messaging Workshop](docs/LANDING_PAGE_MESSAGING_WORKSHOP.md)

## Review Artifacts

- [CEO Plan: Workflow Reliability Control Loop](docs/ceo-plans/2026-03-22-workflow-reliability-control-loop.md)
- [Eng Review: Workflow Reliability Control Loop](docs/eng-reviews/2026-03-23-workflow-reliability-control-loop.md)
- [Design Review: Workflow Reliability Control Loop](docs/design-reviews/2026-03-23-workflow-reliability-control-loop.md)

## Design Spec Assets

- [ClawTrace Atelier Component Spec](docs/design-specs/CLAWTRACE_ATELIER_COMPONENT_SPEC.md)
- [ClawTrace Token Block](docs/design-specs/clawtrace.tokens.css)
- [ClawTrace React Interfaces](docs/design-specs/clawtrace.interfaces.ts)

## Operations

- [TODOs](TODOS.md)
